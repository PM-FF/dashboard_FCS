...
def get_calendar_table(df, col_params, id_idx):
    return dash_table.DataTable(
        data=df.reset_index().to_dict('records'),
        id=f"calendar-table-{id_idx}",
        columns=col_params,
        export_format='xlsx',
        export_headers='display',
        style_table={'height': '600px',
    ...
    tabs_panels = [
        dmc.TabsPanel(
            children=[
                ...
                html.Div(get_date_picker(df, part), className='date-picker-container')
            ],
            value=str(index)
        )
...
def get_delta_action_plot(df, part, id):
    params = {
        'x': 'Date',
        'mode': 'lines',
        'line_shape': 'linear',
...
def get_delta_action_tabs(df):
    ...
    tabs_list = dmc.TabsList([dmc.Tab(label, value=str(index)) for index, label in enumerate(tab_labels)])
    tabs_panels = [
        dmc.TabsPanel(get_delta_action_plot(df[part], part, f'delta-action-graph-{index}'), value=str(index))
        for index, part in enumerate(df.keys())
    ]
...
def create_perf_calendar_table(df, part, id_idx):
    col_params = [
        {'name': '' if col == 'Year' else col, 'id': col}
        for col in df['perf_calendar'][part].columns
    ]
    return get_calendar_table(df['perf_calendar'][part], col_params, id_idx)
...
def get_perf_calendar_tabs(df):
    ...
    tabs_list = dmc.TabsList([dmc.Tab(label, value=str(index)) for index, label in enumerate(tab_labels)])
    tabs_panels = [
        dmc.TabsPanel(create_perf_calendar_table(df, part, index), value=str(index))
